...
x_stem = x * np.cos(stem_rotation) - y * np.sin(stem_rotation)
y_stem = x * np.sin(stem_rotation) + y * np.cos(stem_rotation) - 2 * np.pi / squish
...
total_points = len(x_stem) + len(x_petals) + len(x_seeds)
...
def combined_update(i):
    ...
    if i >= total_points:
        ...
        if end_points > len(x_stem):
            curve_2.set_data(x_petals[:end_points - len(x_stem)], y_petals[:end_points - len(x_stem)])
            curve_3.set_data(x_seeds[:end_points - len(x_stem)], y_seeds[:end_points - len(x_stem)])
        else:
            curve_2.set_data([], [])
            curve_3.set_data([], [])
            curve_1.set_data(x_stem[:end_points], y_stem[:end_points])
    else:
        # We are in the first half of the animation, so plot the points
        if i < len(x_stem):
            curve_1.set_data(x_stem[:i], y_stem[:i])
        else:
            rem_i = i - len(x_stem)
